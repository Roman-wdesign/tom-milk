...
const finalProduct = computed<number>(() =>
  Math.round(volumeMilkReqInput.value * volumeFatReqInput.value)
) // x= i*k
const currentProduct = computed<number>(() =>
  Math.round(volumeMilkAvailInput.value * volumeFatAvailInput.value)
) // y= j*c
const overProduct = computed<number>(() => Math.round(finalProduct.value - currentProduct.value)) // z= x-y
const overVolume = computed<number>(() =>
  Math.round(volumeMilkReqInput.value - volumeMilkAvailInput.value)
) //v= i-j
